...
if __name__ == "__main__":
    model = VASNet_Audio128_Concat()
    model.load_state_dict(torch.load('/home/lol/lol_highlight_ai/model/result/result_vasnet_audio128_concat/models/m2_splits_0_0.5118933946786492.tar.pth'))
    kw = model.att.K.weight
    qw = model.att.Q.weight
    vw = model.att.V.weight

    print('kw vid', kw[:1024, :].abs().mean())
    print('kw aud', kw[1024:, :].abs().mean())
    print('kw vid', qw[:1024, :].abs().mean())
    print('kw aud', qw[1024:, :].abs().mean())
    print('kw vid', vw[:1024, :].abs().mean())
    print('kw aud', vw[1024:, :].abs().mean())