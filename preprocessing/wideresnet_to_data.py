...
def get_wideresnet_pool5(video_name,per_frame,full_space):
    start = time.time()
    ...
    for pic_num in range(0,int(video_length)):
        ...
        if pic_num % full_space == 0:
            ...
            if pic_num % per_frame == 0:
                features.append(pool5['AdaptiveAvgPool2d'].cpu().numpy().reshape(2048))
    end = time.time()
    print(end-start)
...
#-------------------------------------
def revise_to_h5(f,video_name,features=None,audio_features_20=None,audio_features_128=None,gtscore=None,gtsummary=None,n_frame_per_seg=None,n_frames=None,change_point=None,picks=None,n_steps=None,user_summary=None,video_name_original=None,cp_sift=None,n_frame_per_seg_sift=None, features_resnet=None):
...
def one_video_to_h5(h5,path,game,frame_space=15,full_space=5,threshold=0.007,sift_threshold=10,create= False):
    ...
    # Extract Video Features
    #----------------------------------------
    features = get_wideresnet_pool5(vid_name,frame_space,full_space)
    #----------------------------------------
    # Save to h5
    #----------------------------------------
    if create:
...
if __name__ == "__main__":
    ...
    m2 = h5py.File("/home/lol/origin_h5/lol_dataset.h5","a")
