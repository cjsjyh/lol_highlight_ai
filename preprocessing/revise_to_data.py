...
import time
import lol_highlight_ai.model.cpd_auto
from lol_highlight_ai.model.cpd_auto import cpd_auto
import numpy
import h5py
...
def one_video_to_h5(h5,path,games,frame_space=15,full_space=5,threshold=0.007,sift_threshold=10):
    ...
    for game in games:
        with h5py.File(h5,'a') as dataset:
            if game in dataset.keys():
                del dataset[game]
                print(f"{game} already in the dataset. delete and rewrite")
        if '.mp4' in game:
            vid_name = path+game
            print(vid_name)
            ...
            with h5py.File(h5,'a') as dataset:
                if game not in dataset.keys():
                    data_to_h5(h5,game,features=features,n_frame_per_seg=n_frame_per_seg, n_frames=n_frames,change_point=change_points,picks=picks,n_steps=n_steps)
                else:
...
if __name__ == "__main__":
    ...
    games =["20200325_GRF_GEN_1_full.mp4"]
